...
void UAbilityTask_GrantNearbyInteraction::QueryInteractables()
{
	...
	if (World && ActorOwner)
	{
		...
		World->OverlapMultiByChannel(OUT OverlapResults, ActorOwner->GetActorLocation(), FQuat::Identity, InteractionCollisionChannelRegistry::Get(), FCollisionShape::MakeSphere(InteractionScanRange), Params);
