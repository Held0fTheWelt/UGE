...
DEFINE_LOG_CATEGORY(Log_AircraftPawn);

AAircraftPawn::AAircraftPawn(const FObjectInitializer& ObjectInitializer)
{
...
void AAircraftPawn::SetSpeed_Implementation(float Speed)
{
	if (AircraftMovementComponent)
	{
		...
		UE_LOGFMT(Log_AircraftPawn, Verbose, "{Time}: {Line} {Class}: Aircraft speed set to: {Speed}",
	...
	{
		UE_LOGFMT(Log_AircraftPawn, Warning, "{Time}: {Line} {Class}: AircraftMovementComponent is not initialized.",
			("Class", GET_CLASSNAME_WITH_FUNCTION),
			("Line", GET_LINE_NUMBER),
			("Time", GET_CURRENT_TIME));
	}
