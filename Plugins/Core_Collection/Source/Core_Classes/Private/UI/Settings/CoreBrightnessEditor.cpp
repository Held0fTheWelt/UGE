...
#include "UI/Settings/CoreBrightnessEditor.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreBrightnessEditor)
...
UCoreBrightnessEditor::UCoreBrightnessEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UCoreBrightnessEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UCoreBrightnessEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UCoreSettingsLocal::Get()->GetSafeZone());
...
bool UCoreBrightnessEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UCoreBrightnessEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UCoreBrightnessEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UCoreBrightnessEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UCoreBrightnessEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UCoreSettingsLocal::Get()->GetSafeZone());
}

void UCoreBrightnessEditor::HandleDoneClicked()
{
	...
	{
		UCoreSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
