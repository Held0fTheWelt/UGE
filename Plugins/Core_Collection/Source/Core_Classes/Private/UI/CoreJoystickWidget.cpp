...
#include "UI/CoreJoystickWidget.h"

#include "CommonHardwareVisibilityBorder.h"
#include "Components/Image.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreJoystickWidget)

#define LOCTEXT_NAMESPACE "CoreJoystick"

UCoreJoystickWidget::UCoreJoystickWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetConsumePointerInput(true);
}

FReply UCoreJoystickWidget::NativeOnTouchStarted(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
...
FReply UCoreJoystickWidget::NativeOnTouchMoved(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
...
FReply UCoreJoystickWidget::NativeOnTouchEnded(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
	StopInputSimulation();
	return FReply::Handled().ReleaseMouseCapture();
}

void UCoreJoystickWidget::NativeOnMouseLeave(const FPointerEvent& InMouseEvent)
{
	Super::NativeOnMouseLeave(InMouseEvent);
	StopInputSimulation();
}

void UCoreJoystickWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (!CommonVisibilityBorder || CommonVisibilityBorder->IsVisible())
	{
		...
		{
			const FVector2D ScaleVector = bNegateYAxis ? FVector2D(1.0f, -1.0f) : FVector2D(1.0f, 1.0f);
...
void UCoreJoystickWidget::HandleTouchDelta(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
...
void UCoreJoystickWidget::StopInputSimulation()
{
