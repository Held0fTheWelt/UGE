...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreControllerDisconnectedScreen)
...
UCoreControllerDisconnectedScreen::UCoreControllerDisconnectedScreen(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	// By default, only strict pairing platforms will need this button.
	PlatformSupportsUserChangeTags.AddTag(TAG_Platform_Trait_Input_HasStrictControllerPairing.GetTag());
}

void UCoreControllerDisconnectedScreen::NativeOnActivated()
{
	Super::NativeOnActivated();

	if (!HBox_SwitchUser)
	{
		UE_LOG(Core_Classes, Error, TEXT("Unable to find HBox_SwitchUser on Widget %s"), *GetNameSafe(this));
		return;
	}

	if (!Button_ChangeUser)
	{
		UE_LOG(Core_Classes, Error, TEXT("Unable to find Button_ChangeUser on Widget %s"), *GetNameSafe(this));
		return;
	}
...
bool UCoreControllerDisconnectedScreen::ShouldDisplayChangeUserButton() const
{
...
void UCoreControllerDisconnectedScreen::HandleChangeUserClicked()
{
	ensure(ShouldDisplayChangeUserButton());

	UE_LOG(Core_Classes, Log, TEXT("[%hs] Change user requested!"), __func__);
...
void UCoreControllerDisconnectedScreen::HandleChangeUserCompleted(const FPlatformUserSelectionCompleteParams& Params)
{
	UE_LOG(Core_Classes, Log, TEXT("[%hs] User change complete!"), __func__);
