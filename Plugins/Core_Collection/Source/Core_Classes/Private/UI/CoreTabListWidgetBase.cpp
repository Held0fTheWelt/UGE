...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreTabListWidgetBase)

void UCoreTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UCoreTabListWidgetBase::NativeConstruct()
{
...
void UCoreTabListWidgetBase::NativeDestruct()
{
	for (FCoreTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UCoreTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FCoreTabDescriptor& OutTabInfo)
{
	const FCoreTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FCoreTabDescriptor& TabInfo) -> bool
	{
...
void UCoreTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FCoreTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UCoreTabListWidgetBase::RegisterDynamicTab(const FCoreTabDescriptor& TabDescriptor)
{
...
void UCoreTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FCoreTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UCoreTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UCoreTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FCoreTabDescriptor* TabInfoPtr = nullptr;
	
	FCoreTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UCoreTabButtonInterface::StaticClass()))
	{
		...
		{
			ICoreTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UCoreTabListWidgetBase::IsFirstTabActive() const
{
...
bool UCoreTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UCoreTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UCoreTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UCoreTabListWidgetBase::SetupTabs()
{
	for (FCoreTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
