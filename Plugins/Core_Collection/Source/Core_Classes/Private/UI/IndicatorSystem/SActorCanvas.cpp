...
#include "UI/IndicatorSystem/ICoreActorIndicatorWidget.h"
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UCoreIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UCoreIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
		if (IndicatorComponent)
		{
			...
			for (UCoreIndicatorDescriptor* Indicator : IndicatorComponent->GetIndicators())
			{
	...
	if (LocalPlayer)
	{
		...
		if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, /*out*/ ProjectionData))
		{
			...
			for (int32 ChildIndex = 0; ChildIndex < CanvasChildren.Num(); ++ChildIndex)
			{
				...
				UCoreIndicatorDescriptor* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	if (bShowAnyIndicators)
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < SortedSlots.Num(); ++ChildIndex)
		{
			...
			const UCoreIndicatorDescriptor* Indicator = CurChild.Indicator;
...
void SActorCanvas::OnIndicatorAdded(UCoreIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::OnIndicatorRemoved(UCoreIndicatorDescriptor* Indicator)
{
	RemoveIndicatorForEntry(Indicator);
...
void SActorCanvas::AddIndicatorForEntry(UCoreIndicatorDescriptor* Indicator)
{
	...
	UClass* LoadedClass = IndicatorClass.LoadSynchronous();
	if (!LoadedClass)
	{
		return;
	}
	...
	if (UUserWidget* IndicatorWidget = IndicatorPool.GetOrCreateInstance(TSubclassOf<UUserWidget>(LoadedClass)))
	{
		if (IndicatorWidget->GetClass()->ImplementsInterface(UCoreActorIndicatorWidgetInterface::StaticClass()))
		{
			ICoreActorIndicatorWidgetInterface::Execute_BindIndicator(IndicatorWidget, Indicator);
		}
...
void SActorCanvas::RemoveIndicatorForEntry(UCoreIndicatorDescriptor* Indicator)
{
	if (UUserWidget* IndicatorWidget = Indicator->IndicatorWidget.Get())
	{
		if (IndicatorWidget->GetClass()->ImplementsInterface(UCoreActorIndicatorWidgetInterface::StaticClass()))
		{
			ICoreActorIndicatorWidgetInterface::Execute_UnbindIndicator(IndicatorWidget, Indicator);
		}
...
SActorCanvas::FScopedWidgetSlotArguments SActorCanvas::AddActorSlot(UCoreIndicatorDescriptor* Indicator)
{
...
void SActorCanvas::GetOffsetAndSize(const UCoreIndicatorDescriptor* Indicator,
	FVector2D& OutSize, 
	FVector2D& OutOffset,
