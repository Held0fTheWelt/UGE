...
#include "UI/CoreSimulatedInputWidget.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreSimulatedInputWidget)

#define LOCTEXT_NAMESPACE "CoreSimulatedInputWidget"

UCoreSimulatedInputWidget::UCoreSimulatedInputWidget(const FObjectInitializer& ObjectInitializer)
...
#if WITH_EDITOR
const FText UCoreSimulatedInputWidget::GetPaletteCategory()
{
	return LOCTEXT("PalleteCategory", "Input");
}
#endif // WITH_EDITOR

void UCoreSimulatedInputWidget::NativeConstruct()
{
	...
	{
		System->ControlMappingsRebuiltDelegate.AddUniqueDynamic(this, &UCoreSimulatedInputWidget::OnControlMappingsRebuilt);
	}
	
	Super::NativeConstruct();
}

void UCoreSimulatedInputWidget::NativeDestruct()
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
FReply UCoreSimulatedInputWidget::NativeOnTouchEnded(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
	FlushSimulatedInput();
...
UEnhancedInputLocalPlayerSubsystem* UCoreSimulatedInputWidget::GetEnhancedInputSubsystem() const
{
...
UEnhancedPlayerInput* UCoreSimulatedInputWidget::GetPlayerInput() const
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
void UCoreSimulatedInputWidget::InputKeyValue(const FVector& Value)
{
	...
	{
		UE_LOG(Core_Classes, Error, TEXT("'%s' is attempting to simulate input but has no player input!"), *GetNameSafe(this));
	}
}

void UCoreSimulatedInputWidget::InputKeyValue2D(const FVector2D& Value)
{
	InputKeyValue(FVector(Value.X, Value.Y, 0.0));
}

void UCoreSimulatedInputWidget::FlushSimulatedInput()
{
...
void UCoreSimulatedInputWidget::QueryKeyToSimulate()
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
void UCoreSimulatedInputWidget::OnControlMappingsRebuilt()
{
	QueryKeyToSimulate();
}
