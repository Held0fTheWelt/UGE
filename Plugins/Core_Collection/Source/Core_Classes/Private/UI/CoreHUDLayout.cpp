...
#include "UI/CoreControllerDisconnectedScreen.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreHUDLayout)
...
UCoreHUDLayout::UCoreHUDLayout(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
	, SpawnedControllerDisconnectScreen(nullptr)
{
	// By default, only primarily controller platforms require a disconnect screen. 
	PlatformRequiresControllerDisconnectScreen.AddTag(TAG_Platform_Trait_Input_PrimarlyController.GetTag());
}

void UCoreHUDLayout::NativeOnInitialized()
{
	Super::NativeOnInitialized();

	RegisterUIActionBinding(FBindUIActionArgs(FUIActionTag::ConvertChecked(TAG_UI_ACTION_ESCAPE.GetTag()), false, FSimpleDelegate::CreateUObject(this, &ThisClass::HandleEscapeAction)));
...
void UCoreHUDLayout::NativeDestruct()
{
...
void UCoreHUDLayout::HandleEscapeAction()
{
	if (ensure(!EscapeMenuClass.IsNull()))
	{
		UCommonUIExtensions::PushStreamedContentToLayer_ForPlayer(GetOwningLocalPlayer(), TAG_UI_LAYER_MENU.GetTag(), EscapeMenuClass);
	}
}

void UCoreHUDLayout::HandleInputDeviceConnectionChanged(EInputDeviceConnectionState NewConnectionState, FPlatformUserId PlatformUserId, FInputDeviceId InputDeviceId)
{
...
void UCoreHUDLayout::HandleInputDevicePairingChanged(FInputDeviceId InputDeviceId, FPlatformUserId NewUserPlatformId, FPlatformUserId OldUserPlatformId)
{
	const FPlatformUserId OwningLocalPlayerId = GetOwningLocalPlayer()->GetPlatformUserId();
...
bool UCoreHUDLayout::ShouldPlatformDisplayControllerDisconnectScreen() const
{
...
void UCoreHUDLayout::NotifyControllerStateChangeForDisconnectScreen()
{
...
void UCoreHUDLayout::ProcessControllerDevicesHavingChangedForDisconnectScreen()
{
	// We should only ever get here if we have bound to the controller state change delegates
	ensure(ShouldPlatformDisplayControllerDisconnectScreen());
...
void UCoreHUDLayout::DisplayControllerDisconnectedMenu_Implementation()
{
	UE_LOG(Core_Classes, Log, TEXT("[%hs] Display controller disconnected menu!"), __func__);

	if (ControllerDisconnectedScreen)
	{
		// Push the "controller disconnected" widget to the menu layer
		SpawnedControllerDisconnectScreen = UCommonUIExtensions::PushContentToLayer_ForPlayer(GetOwningLocalPlayer(), TAG_UI_LAYER_MENU.GetTag(), ControllerDisconnectedScreen);
	}
}

void UCoreHUDLayout::HideControllerDisconnectedMenu_Implementation()
{
	UE_LOG(Core_Classes, Log, TEXT("[%hs] Hide controller disconnected menu!"), __func__);
