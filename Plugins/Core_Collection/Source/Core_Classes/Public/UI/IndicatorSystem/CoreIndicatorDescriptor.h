...
#include "CoreIndicatorDescriptor.generated.h"
...
{
	bool Project(const UCoreIndicatorDescriptor& IndicatorDescriptor, const FSceneViewProjectionData& InProjectionData, const FVector2f& ScreenSize, FVector& ScreenPositionWithDepth);
};
 ...
 * ICoreActorIndicatorWidgetInterface so that it can 'bind' to the associated data.
 */
UCLASS(BlueprintType)
class CORE_CLASSES_API UCoreIndicatorDescriptor : public UObject
{
	GENERATED_BODY()
	
public:
	UCoreIndicatorDescriptor() { }
...
public:
	UCoreIndicatorManagerComponent* GetIndicatorManagerComponent() { return ManagerPtr.Get(); }
	void SetIndicatorManagerComponent(UCoreIndicatorManagerComponent* InManager);
...
private:
	...
	UPROPERTY()
	TWeakObjectPtr<UCoreIndicatorManagerComponent> ManagerPtr;
