...
/** SActorCanvas uses SWidget::RegisterActiveTimer (no FAsyncMixin dependency). */
...
public:
	...
	public: 
		...
		FSlot(UCoreIndicatorDescriptor* InIndicator)
			: TSlotBase<FSlot>()
	...
	private:
		...
		UCoreIndicatorDescriptor* Indicator;
...
private:
	void OnIndicatorAdded(UCoreIndicatorDescriptor* Indicator);
	void OnIndicatorRemoved(UCoreIndicatorDescriptor* Indicator);

	void AddIndicatorForEntry(UCoreIndicatorDescriptor* Indicator);
	void RemoveIndicatorForEntry(UCoreIndicatorDescriptor* Indicator);

	using FScopedWidgetSlotArguments = TPanelChildren<FSlot>::FScopedWidgetSlotArguments;
	FScopedWidgetSlotArguments AddActorSlot(UCoreIndicatorDescriptor* Indicator);
	...
	void GetOffsetAndSize(const UCoreIndicatorDescriptor* Indicator,
		FVector2D& OutSize, 
		FVector2D& OutOffset,
...
private:
	TArray<TObjectPtr<UCoreIndicatorDescriptor>> AllIndicators;
	TArray<UCoreIndicatorDescriptor*> InactiveIndicators;
	
	FLocalPlayerContext LocalPlayerContext;
	TWeakObjectPtr<UCoreIndicatorManagerComponent> IndicatorComponentPtr;
