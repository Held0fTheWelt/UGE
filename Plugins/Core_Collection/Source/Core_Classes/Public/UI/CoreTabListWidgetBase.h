...
#include "CoreTabListWidgetBase.generated.h"
...
class UCoreTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class ICoreTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FCoreTabDescriptor& TabDescriptor);
};

/**
 * UCoreTabListWidgetBase
 *
 * Tab list widget base with tab descriptor support.
 * Adapted from ULyraTabListWidgetBase.
 */
UCLASS(Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class CORE_CLASSES_API UCoreTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
bool GetPreregisteredTabInfo(const FName TabNameId, FCoreTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FCoreTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
...
void SetTabHiddenState(FName TabNameId, bool bHidden);
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
bool RegisterDynamicTab(const FCoreTabDescriptor& TabDescriptor);
...
int32 GetVisibleTabCount();
	...
	DECLARE_EVENT_TwoParams(UCoreTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
void SetupTabs();
	...
	TArray<FCoreTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FCoreTabDescriptor> PendingTabLabelInfoMap;
};
