...
void UAsyncAction_QueryReplays::Activate()
{
	...
	ResultList = NewObject<UWarCollectionReplayList>();
...
void UAsyncAction_QueryReplays::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result)
{
	...
	{
		UWarCollectionReplayListEntry* NewReplayEntry = NewObject<UWarCollectionReplayListEntry>(ResultList);
		NewReplayEntry->StreamInfo = StreamInfo;
		ResultList->Results.Add(NewReplayEntry);
	}

	// Sort demo names by date
	Algo::SortBy(ResultList->Results, [](const TObjectPtr<UWarCollectionReplayListEntry>& Data) { return Data->StreamInfo.Timestamp.GetTicks(); }, TGreater<>());
