...
UGlobalAbilitySubsystem::UGlobalAbilitySubsystem()
{
}
...
void UGlobalAbilitySubsystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		...
		for (UCoreAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UGlobalAbilitySubsystem::RegisterASC(UCoreAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UGlobalAbilitySubsystem::UnregisterASC(UCoreAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
