...
DEFINE_LOG_CATEGORY(Log_AbilitySystem_CueManager);

#include UE_INLINE_GENERATED_CPP_BY_NAME(GameplayCueManager_Core)
...
namespace LyraGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		...
		FConsoleCommandWithArgsDelegate::CreateStatic(UGameplayCueManager_Core::DumpGameplayCues));
...
UGameplayCueManager_Core::UGameplayCueManager_Core(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGameplayCueManager_Core* UGameplayCueManager_Core::Get()
{
	return Cast<UGameplayCueManager_Core>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGameplayCueManager_Core::OnCreated()
{
	Super::OnCreated();
...
void UGameplayCueManager_Core::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(Log_AbilitySystem_CueManager, Warning, TEXT("ULyraGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGameplayCueManager_Core::ShouldAsyncLoadRuntimeObjectLibraries() const
{
...
bool UGameplayCueManager_Core::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGameplayCueManager_Core::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGameplayCueManager_Core::DumpGameplayCues(const TArray<FString>& Args)
{
	UGameplayCueManager_Core* GCM = Cast<UGameplayCueManager_Core>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(Log_AbilitySystem_CueManager, Error, TEXT("DumpGameplayCues failed. No ULyraGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(Log_AbilitySystem_CueManager, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGameplayCueManager_Core::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGameplayCueManager_Core* StrongThis = Get())
					{
...
void UGameplayCueManager_Core::HandlePostGarbageCollect()
{
...
void UGameplayCueManager_Core::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(Log_AbilitySystem_CueManager, Warning, TEXT("ULyraGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGameplayCueManager_Core::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (LyraGameplayCueManagerCvars::LoadMode)
	{
	case ELyraEditorLoadMode::LoadUpfront:
...
void UGameplayCueManager_Core::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGameplayCueManager_Core::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGameplayCueManager_Core::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGameplayCueManager_Core::UpdateDelayLoadDelegateListeners()
{
...
bool UGameplayCueManager_Core::ShouldDelayLoadGameplayCues() const
{
...
void UGameplayCueManager_Core::RefreshGameplayCuePrimaryAsset()
{
