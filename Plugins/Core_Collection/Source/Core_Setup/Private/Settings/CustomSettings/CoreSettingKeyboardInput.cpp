...
#include "Settings/CustomSettings/CoreSettingKeyboardInput.h"

#include "EnhancedInputSubsystems.h"
#include "PlayerMappableInputConfig.h"
#include "UserSettings/EnhancedInputUserSettings.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(CoreSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "CoreSettings"

namespace Core::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("CoreErrors_UnknownMappingName", "Unknown Mapping");
}

UCoreSettingKeyboardInput::UCoreSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UCoreSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return Core::ErrorMessages::UnknownMappingName;
}

FText UCoreSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UCoreSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UCoreSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UCoreSettingKeyboardInput::GetUserSettings() const
{
...
void UCoreSettingKeyboardInput::OnInitialized()
{
...
void UCoreSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UCoreSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
FText UCoreSettingKeyboardInput::GetKeyTextFromSlot(int32 SlotIndex) const
{
	return GetKeyTextFromSlot(static_cast<EPlayerMappableKeySlot>(SlotIndex));
}

void UCoreSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UCoreSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UCoreSettingKeyboardInput::RestoreToInitial()
{
...
bool UCoreSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UCoreSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UCoreSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
