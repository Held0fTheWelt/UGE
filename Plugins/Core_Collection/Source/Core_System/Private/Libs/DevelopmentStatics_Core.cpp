...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DevelopmentStatics_Core)

bool UDevelopmentStatics_Core::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
...
bool UDevelopmentStatics_Core::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
...
bool UDevelopmentStatics_Core::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
...
UWorld* UDevelopmentStatics_Core::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UDevelopmentStatics_Core::GetAllBlueprints()
{
...
UClass* UDevelopmentStatics_Core::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UDevelopmentStatics_Core::GetAllBlueprints();
...
UClass* UDevelopmentStatics_Core::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
