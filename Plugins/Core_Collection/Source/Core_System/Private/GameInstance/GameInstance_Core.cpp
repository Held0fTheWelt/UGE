...
#include "GameInstance/GameInstance_Core.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GameInstance_Core)
...
UGameInstance_Core::UGameInstance_Core(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UGameInstance_Core::OnExperienceLoaded(const UObject* LoadedExperience)
{
	CoreGameInstanceDelegates::BroadcastExperienceLoaded(LoadedExperience);
}

bool UGameInstance_Core::DoesPlatformSupportReplays() const
{
	return CoreGameInstanceDelegates::DoesPlatformSupportReplays();
}

void UGameInstance_Core::Init()
{
	Super::Init();
	...
	{
		//ComponentManager->RegisterInitState(GameplayTags_Core::InitState_Spawned, false, FGameplayTag());
		//ComponentManager->RegisterInitState(GameplayTags_Core::InitState_DataAvailable, false, GameplayTags_Core::InitState_Spawned);
		//ComponentManager->RegisterInitState(GameplayTags_Core::InitState_DataInitialized, false, GameplayTags_Core::InitState_DataAvailable);
		//ComponentManager->RegisterInitState(GameplayTags_Core::InitState_GameplayReady, false, GameplayTags_Core::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UGameInstance_Core::OnPreClientTravelToSession);
	}
}

void UGameInstance_Core::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
//ALyraPlayerController* UGameInstance_Core::GetPrimaryPlayerController() const
//{
//	return Cast<ALyraPlayerController>(Super::GetPrimaryPlayerController(false));
//}

bool UGameInstance_Core::CanJoinRequestedSession() const
{
...
void UGameInstance_Core::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
...
void UGameInstance_Core::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
...
void UGameInstance_Core::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UGameInstance_Core::OnPreClientTravelToSession(FString& URL)
{
