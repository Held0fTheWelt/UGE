...
#include "CoreReplicationGraphTypes.generated.h"

// This is the main enum we use to route actors to the right replication node. Each class maps to one enum.
UENUM()
enum class ECoreClassRepNodeMapping : uint32
{
	NotRouted,						// Doesn't map to any node. Used for special case actors that handled by special case nodes (UCoreReplicationGraphNode_PlayerStateFrequencyLimiter)
	RelevantAllConnections,			// Routes to an AlwaysRelevantNode or AlwaysRelevantStreamingLevelNode node

	// ONLY SPATIALIZED Enums below here! See UCoreReplicationGraph::IsSpatialized
...
USTRUCT()
struct CORE_SYSTEM_API FCoreRepGraphActorClassSettings
{
	GENERATED_BODY()

	FCoreRepGraphActorClassSettings() = default;
	...
	ECoreClassRepNodeMapping ClassNodeMapping = ECoreClassRepNodeMapping::NotRouted;
	...
	UClass* GetStaticActorClass() const
	{
		...
		if (FPackageName::IsScriptPackage(ActorClassNameString))
		{
			...
			if (!StaticActorClass)
			{
				UE_LOG(LogTemp, Error, TEXT("FCoreRepGraphActorClassSettings: Cannot Find Static Class for %s"), *ActorClassNameString);
			}
		}
		else
		{
			...
			if (!StaticActorClass)
			{
				UE_LOG(LogTemp, Error, TEXT("FCoreRepGraphActorClassSettings: Cannot Load Static Class for %s"), *ActorClassNameString);
			}
		}
