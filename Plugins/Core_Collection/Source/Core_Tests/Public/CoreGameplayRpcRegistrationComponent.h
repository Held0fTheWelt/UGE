...
#include "CoreGameplayRpcRegistrationComponent.generated.h"

#define UE_API CORE_TESTS_API


UCLASS(MinimalAPI)
class UCoreGameplayRpcRegistrationComponent : public UExternalRpcRegistrationComponent
{
	GENERATED_BODY()
protected:
	static UE_API UCoreGameplayRpcRegistrationComponent* ObjectInstance;

public:
	static UE_API UCoreGameplayRpcRegistrationComponent* GetInstance();
...
// These are RPCs that should only be enabled while we are in the frontend.
	...
	UE_API bool HttpGetMatchmakingStatus(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete);
	UE_API bool HttpPostMatchmakingFind(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete);
